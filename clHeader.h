...
int get_local_id(int dim) { return 0; }
...
{
	unsigned int i = get_global_id(0);//(blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (i >= N) return;
	int out_index = i;
...
float _linear_activate(float x) { return x; }
float _logistic_activate(float x) { return 1. / (1. + exp(-x)); }
float _loggy_activate(float x) { return 2. / (1. + exp(-x)) - 1; }
float _relu_activate(float x) { return x*(x>0); }
float _elu_activate(float x) { return (x >= 0)*x + (x < 0)*(exp(x) - 1); }
float _selu_activate(float x) { return (x >= 0)*1.0507*x + (x < 0)*1.0507*1.6732*(exp(x) - 1); }
float _relie_activate(float x) { return (x>0) ? x : .01*x; }
float _ramp_activate(float x) { return x*(x>0) + .1*x; }
float _leaky_activate(float x) { return (x>0) ? x : .1*x; }
float _tanh_activate(float x) { return (exp(2 * x) - 1) / (exp(2 * x) + 1); }

float _plse_activate(float x)
{
...
float _lhtan_activate(float x)
{
	if (x < 0) return .001*x;
	if (x > 1) return .001*(x - 1) + 1;
	return x;
}
float _lhtan_gradient(float x)
{
	if (x > 0 && x < 1) return 1;
	return .001;
}

float _hardtan_gradient(float x)
{
	if (x > -1 && x < 1) return 1;
	return 0;
}
...
float _stair_activate(float x)
{
	int n = floor(x);
	if (n % 2 == 0) return floor(x / 2.0f);
	else return (x - n) + floor(x / 2.0f);
}
float _hardtan_activate(float x)
{
	if (x < -1) return -1;
	if (x > 1) return 1;
	return x;
...
kernel
#endif
void kernel_activate(Float_ptr x, int a, int n)
{
	int i = get_global_id(0);
	...
	switch (a) {
	case LINEAR:
		x[i] = _linear_activate(x[i]);
	case LOGISTIC:
		x[i] = _logistic_activate(x[i]);
	case LOGGY:
		x[i] = _loggy_activate(x[i]);
	case RELU:
		x[i] = _relu_activate(x[i]);
	case ELU:
		x[i] = _elu_activate(x[i]);
	case SELU:
		x[i] = _selu_activate(x[i]);
	case RELIE:
		x[i] = _relie_activate(x[i]);
	case RAMP:
		x[i] = _ramp_activate(x[i]);
	case LEAKY:
		x[i] = _leaky_activate(x[i]);
	case TANH:
		x[i] = _tanh_activate(x[i]);
	case PLSE:
		x[i] = _plse_activate(x[i]);
	case STAIR:
		x[i] = _stair_activate(x[i]);
	case HARDTAN:
		x[i] = _hardtan_activate(x[i]);
	case LHTAN:
		x[i] = _lhtan_activate(x[i]);
	}
	return;
}
...
void activate(Float_ptr x, const int n)
{
	int i;
	for (i = 0; i < n; ++i) {
		x[i] = _logistic_activate(x[i]);
...
kernel
#endif
void kernel_yolo_layer(Float_ptr l_output,int l_n, int l_w, int l_h, int l_classes, int l_outputs)
{
	int gi = get_global_id(0);
	int b = gi / l_n;
	int n = gi % l_n;

	int index = entry_indexF(l_w,l_h,l_classes,l_outputs, b, n*l_w*l_h, 0);
	activate(&l_output[index], 2 * l_w*l_h);
	index = entry_indexF(l_w, l_h, l_classes, l_outputs, b, n*l_w*l_h, 4);
	activate(&l_output[index], (1 + l_classes)*l_w*l_h);
...
void kernel_yolo_layerAcc(Float_ptr l_output, Int_ptr indexes, Int_ptr lengths, int lnlb)
{
	...
	for (int i = 0; i < lnlb; i++)
	{
		...
		{
			l_output[index] = _logistic_activate(l_output[index]);
			return;
		}
	}
