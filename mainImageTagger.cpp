...
void drawBlobsOnFrame(std::vector<trackingLib::Blob*>& tBs, cv::Mat& frame, int nFrame, cv::Scalar blobColor,trackingLib::clMultiTracker* _multiTracker)
{
	...
	for (auto b : tBs)
	{
		...
		trackingLib::classToTrack* ctk = _multiTracker->getClass(b->classes[0].first);
		...
		if (b->classes.size() > 0)
		{
			s = std::to_string(b->id) + ":" + b->classes[0].first + ":" + to_string(b->life) + ":" + to_string(b->tag);
		}
		else
		{
...
int testImageDetection(std::string dirEXE)
{
 ...
 /////////////////////////////////////////////////
	...
	//// CNN PARAMETERS
	//////////////////////////////////////////////
	std::string cfgfile = "D:\\Resources\\CNNs\\res152_256x192.cfg";
	std::string weightfile = "D:\\Resources\\CNNs\\res152_256x192.weights";
	...
	cv::VideoCapture cap("D:\\Resources\\video\\interview1.mp4");
	...
	for (int i = 0; i < 20; i++)
	{
		...
		trackingLib::BlobsByFrame* bb0 = cnn->readResults();
...
int main(int argc, char **argv) try
{
	...
	std::string videoFN = "";
	cv::VideoCapture cap;
	std::string dirEXE = "D:\\Sdks\\YoloV4\\darknet\\";
	...
	YoloCL::initYoloCL("", 256, gpu_index, 1);
