...
kernel
#endif
void kernel_copy_offset(int N, Float_ptr X, int INCX, Float_ptr Y, int INCY, int offsetY, int offsetX)
{
	int i = get_global_id(0);
	if (i < N) Y[i*INCY + offsetY] = X[i*INCX + offsetX];
}
...
float loggy_activate(float x) { return 2.f / (1.f + exp(-x)) - 1; }
float relu_activate(float x) { return x * (x > 0); }
float relu6_activate(float x) { return min(max(x, 0.0f), 6.0f); }
float elu_activate(float x) { return (x >= 0)*x + (x < 0)*(exp(x) - 1); }
float selu_activate(float x) { return (x >= 0)*1.0507f*x + (x < 0)*1.0507f*1.6732f*(exp(x) - 1); }
float relie_activate(float x) { return (x > 0) ? x : .01f*x; }
float ramp_activate(float x) { return x * (x > 0) + .1f*x; }
float leaky_activate(float x) { return (x > 0) ? x : .1f*x; }
//static inline float tanh_activate(float x){return (exp(2*x)-1)/(exp(2*x)+1);}
float tanh_activate(float x) { return (2 / (1 + exp(-2 * x)) - 1); }
float gelu_activate(float x) { return (0.5*x*(1 + tanh(0.797885*x + 0.035677*pow(x, 3)))); }

float softplus_activate(float x, float threshold) {
	if (x > threshold) return x;                // too large
	else if (x < -threshold) return exp(x);    // too small
	return log(exp(x) + 1);
}
...
float relu6_gradient(float x) { return (x > 0 && x < 6); }
float elu_gradient(float x) { return (x >= 0) + (x < 0)*(x + 1); }
float selu_gradient(float x) { return (x >= 0)*1.0507f + (x < 0)*(x + 1.0507f*1.6732f); }
float relie_gradient(float x) { return (x > 0) ? 1 : .01f; }
float ramp_gradient(float x) { return (x > 0) + .1f; }
float leaky_gradient(float x) { return (x > 0) ? 1 : .1f; }
float tanh_gradient(float x) { return 1 - x * x; }

float sech(float x) { return 2 / (exp(x) + exp(-x)); }
float gelu_gradient(float x) {
	const float x3 = pow(x, 3);
	return 0.5*tanh(0.0356774*x3 + 0.797885*x) + (0.0535161*x3 + 0.398942*x) * pow(sech(0.0356774*x3 + 0.797885*x), 2) + 0.5;
}
float plse_gradient(float x) { return (x < 0 || x > 1) ? .01f : .125f; }
...
kernel
#endif
void kernel_activate_mish(Float_ptr x, int n)
{
	int i = get_global_id(0);
	if (i >= n) return;
	float MISH_THRESHOLD = 20;
	float x_val = x[i];
	
	x[i] = x_val * tanh_activate(softplus_activate(x_val, MISH_THRESHOLD));
...
kernel
#endif
void kernel_shortcut(int batch, int w1, int h1, int c1, Float_ptr add,
	int w2, int h2, int c2, float s1, float s2, Float_ptr out)
{
	int id = get_global_id(0);
...
kernel
#endif
void kernel_shortcut_v2(Float_ptr state_input, Float_ptr layer_before, int size, Float_ptr output)
{
	int id = get_global_id(0);
	...
	if (id < size)
	{
		output[id] = state_input[id] + layer_before[id];
	}
